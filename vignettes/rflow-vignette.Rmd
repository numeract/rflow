...
if (interactive()) {
    ...
    app <- shinyApp(
        ...
        server = function(input, output) {
            mtcars_head <- head(mtcars)
            fn_add_x <- function(df, x) {
                df$mpg <- df$mpg + x
                df
            }
            fn_add_y <- function(df, y) {
                df$cyl <- df$cyl + y
                df
            }
            rv_fn_add_x <- reactive({
                mtcars_head <- fn_add_x(mtcars_head, input$x)
            })
            rv_fn_add_y <- reactive({
                rv_fn_add_x()
                mtcars_head <- fn_add_y(mtcars_head, input$y)
            })
            rv_plot <- reactive({
                mtcars_head <- mtcars_head %>%
                  flow_fn(input$x, fn = rv_fn_add_x) %>%
                  flow_fn(input$y, fn = rv_fn_add_y) %>%
                  collect()
              mtcars_plot <- ggplot2::aes(x = mtcars_head$cyl,
                                          y = mtcars_head$mpg) +
                             ggplot2::geom_point()
              mtcars_plot
            })
            output$plot <- renderPlot(rv_plot())
        }
    )
