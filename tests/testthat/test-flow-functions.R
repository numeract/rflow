...
test_that("make_flow_fn() works with cache re-use", {
    ...
    expect_message(
        ...
        "Reusing cache for function test_fn")
    forget(rflow_test)
})
...
# test_that("make_flow_fn() works with different options, no id", {
#     
#     test_make_flow_fn <- make_flow_fn(test_fn)
#     rflow_test <- test_make_flow_fn(2, 3)
#    
...
test_that("flow_call() works", {
    test_make_flow_fn <- flow_call(test_fn(x = 1, y = 2))
    collected_result <- test_make_flow_fn %>% collect()
